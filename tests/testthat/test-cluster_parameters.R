...
Sigma <- matrix(c(0.03803940, -0.02931614, -0.00766775,
                  -0.02931614,  0.02263954,  0.00586149,
                  -0.00766775,  0.00586149,  0.00167289),
                nrow = 3, ncol = 3)
Sigma.15 <- matrix(c(0.00551067, -0.00437538, -0.00097776,
...
tau.15 <- c(1.3483357195, 2.2472261991, 2.2472261991, 2.2472261991, 2.2472261991,
...
test_that("test Sigma_star_cluster", {
  expect_equal(
    Sigma_star_cluster(Y, X, delta, v_0, alpha, omega_0, mu_0, tau_0,
                       expectation_b, cluster),
    Sigma
  )
  expect_equal(
    Sigma_star_cluster(log(T.15), X, delta, v_0, alpha.15, omega_0, mu_0, tau_0,
...
test_that("test mu_star_cluster", {
  expect_equal(
    mu_star_cluster(Y, X, delta, mu_0, v_0, alpha, omega, mu_0, Sigma,
                    tau_0, expectation_b, cluster),
    mu
  )
  expect_equal(
    mu_star_cluster(log(T.15), X, delta, mu_0, v_0, alpha.15, omega.15,
...
test_that("test sigma_star_cluster", {
  expect_equal(sigma_squared_star(Y, X, delta, alpha, omega, mu, tau_0,
                                  lambda_0, eta_0, expectation_b, cluster),
               sigma_squared)
  expect_equal(sigma_squared_star(log(T.15), X, delta, alpha.15, omega.15,
...
test_that("test tau_star_cluster", {
  expect_equal(tau_star(Y, X, delta, alpha, omega, mu, tau_0, sigma_squared,
                        expectation_b, cluster),
               tau)
  expect_equal(tau_star(log(T.15), X, delta, alpha.15, omega.15, mu.15, tau_0,
                        sigma_squared.15, expectation_b.15, cluster),
              tau.15)
})

test_that("test omega_star_cluster", {
  expect_equal(omega_star_cluster(Y, X, delta, omega_0, mu, tau,
                                  expectation_b, cluster),
               54.3301043)
  expect_equal(omega_star_cluster(log(T.15), X, delta, omega_0, mu.15, tau.15,
                                  expectation_b.15, cluster),
               37.8544658)
})
