...
lung$status <- ifelse(
  lung$`death due to lung cancer (all years)` == "Alive", 0, 1
)
lung$smoking <- ifelse(lung$`smoking pack years` == 0, 0, 1)

# miRNA data
lung2 <-
  df2[df2$race == "AA" & df2$`survival (months)` != "Unknown" &
        df2$`survival (months)` != "0.0", ]
lung2$age <- as.numeric(lung2$age)
lung2$time <- as.numeric(lung2$`survival (months)`) * 30
lung2$status <- 1
lung2$smoking <- ifelse(
  (lung2$`smoking pack year` == 0 | lung2$`smoking pack year` == 0.0), 0, 1
)
